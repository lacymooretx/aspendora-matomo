...
|---|---|---|---|
| `ASPENDORA_GHL_TOKEN` | GoHighLevel Private Integration token (`pit-…`) used by the hourly identity sync to enrich contacts and push tags | GHL sub-account → Settings → Integrations → Private Integrations. Scopes required: `contacts.readonly`, `contacts.write` | Recommended every 90 days (does not auto-expire) |
